...
int is_image_valid(struct elf_info *info) {
    if (sys_memcmp(info->hdr->e_ident, ELFMAG, SELFMAG) != 0) return 0;
    if (info->hdr->e_ident[EI_CLASS] != ELFCLASS64) return 0;
    if (info->hdr->e_machine != EM_AARCH64) return 0; 
    if (info->hdr->e_type != ET_DYN) return 0;
    if (info->hdr->e_phnum <= 0) return 0;
    return 1;
}
...
int load_image(char *elf_start, struct elf_info *info, struct load_segment *load) {
    ...
    if (tls_phdr) {
        size_t headroom = 0x4000;     
        size_t tcb_size  = 0x2000;    
        ...
        void *raw = sys_mmap(NULL, headroom + tls_area + tcb_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        void *tp = (void *)((uintptr_t)raw + headroom + tls_area);
        void *tls_block = (void *)((uintptr_t)tp - tls_area);

        sys_memcpy(tls_block, (void *)(info->pie_base + tls_phdr->p_vaddr), tls_phdr->p_filesz);
        sys_memset((char *)tls_block + tls_phdr->p_filesz, 0, tls_phdr->p_memsz - tls_phdr->p_filesz);
